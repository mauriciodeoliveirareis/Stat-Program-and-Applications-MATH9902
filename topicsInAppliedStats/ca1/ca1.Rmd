...
*Comment*  
I'm not 100% if I can trust this model because, for mutual.role, with the model including all parms we got the following warning message:  
> Model statistics ‘mutual.role’ are not varying. This may indicate that the 
> observed data occupies an extreme point in the sample space or that the 
> estimation has reached a dead-end configuration.  
  

Then we got NA values for Std. Error, MCMC %, z value and Pr(>|z|). Is this because we added both reciprocity and reciprocity with the same role? This model seems broken and I don't think I can make an reliable analysis on this. Even though, let's see how goodness of fit performs with this model:
```{r}
gof(mle.ergmodel_1)
```
...
Now, as previous model had issues with mutual, let's try to drop one of the reciprocities and keep just the one for actors with the same role:

```{r}
ergmodel_2 <- update(ergmodel_1,.~.-mutual)
mle.ergmodel_2 <- ergm(ergmodel_2)
summary(mle.ergmodel_2)

```
  
Now we don't seem to have issues on the reciprocity with the same role.  
```{r}
# checks goodness of fit of the new model
gof(mle.ergmodel_2)

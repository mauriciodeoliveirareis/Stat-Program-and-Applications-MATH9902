...
$$ \eta_i = \beta_0 + \beta_1 (monthlycharges_i) + \beta_2 (sexMale_i) + \beta_3 (partnerYes_i) + \\
\beta_4 (payCreditCard_i) + \beta_5 (payEletronicCheck_i) + \beta_6 (payMailedCheck_i) + \\
\beta_7 (serviceFiberOptic_i) + \beta_8 (serviceNo_i) $$  
...
```{r}
#transform lof odds into probability:
...
- Model 2:
$$ \eta_i = \beta_0 + \beta_1 (monthlycharges_i) + \beta_2 (partnerYes_i) + \beta_3 (payCreditCard_i) + \beta_4 (payEletronicCheck_i) + \\
\beta_5 (payMailedCheck_i) + \beta_6 (serviceFiberOptic_i) + \beta_7 (serviceNo_i) $$  
...
Let's first take a look into the payment method factor levels with high P-value, Credit card and Mailed check and how they compare with bank transfer.  
...
- Odds Ratio for a customer using bank transfer vs. a customer using electronic check
$$logOddsRatio= -\beta_4 = -1.71683$$
$$ oddsRatio= e^{-1.71683}=0.1796347 $$
TODO continue doing odds ration for remaining categoricals and do chart about different fees for services